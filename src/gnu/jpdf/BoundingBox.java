...
public class BoundingBox extends Rectangle {
	...
	private static final int[] VERT_ALIGNS = { VERT_ALIGN_CENTER, VERT_ALIGN_TOP, VERT_ALIGN_BOTTOM };
	
	private static final int[] HORIZ_ALIGNS = { HORIZ_ALIGN_CENTER, HORIZ_ALIGN_LEFT, HORIZ_ALIGN_RIGHT };
	
	private static final int[] SUBTRACTS = { SUBTRACT_FROM_TOP, SUBTRACT_FROM_BOTTOM, SUBTRACT_FROM_LEFT, SUBTRACT_FROM_RIGHT };
	...
	public void drawWrappedString(Graphics g, FontMetrics fm, int padding, int hAlign) throws IllegalArgumentException, StringTooLongException {
		if (getStringArray() == null) {
			...
			for (int i = 0; i < len; i++) {
				...
				wrappedBox = getStringBounds(stringArray[i], hAlign, BoundingBox.VERT_ALIGN_TOP, fm, 0);
	...
	public void drawWrappedStringTruncate(Graphics g, FontMetrics fm, int padding, int hAlign) {
		...
		if (getStringArray() == null) {
			...
			for (int i = 0; i < len; i++) {
				BoundingBox wrappedBox = null;
				try {
					wrappedBox = getStringBounds(stringArray[i], hAlign, BoundingBox.VERT_ALIGN_TOP, fm, 0, false);
	...
	public void drawChoppedString(Graphics g, FontMetrics fm, int padding, int hAlign) {
		...
		try {
			choppedBox = getStringBounds(string, hAlign, VERT_ALIGN_TOP, fm, padding);
			...
			while ((fm.stringWidth(buf.toString()) > (int) getWidth()) && (buf.length() > 0)) {
				buf.deleteCharAt(buf.length() - 1);
			}
			
			try {
				choppedBox = getStringBounds(buf.toString(), hAlign, VERT_ALIGN_TOP, fm, padding);
